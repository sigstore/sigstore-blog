...
To harden our public-good infrastructure and reduce attack surfaces, 
**we are reducing the number of supported entry types on the Sigstore public deployment transparency log.**

**Effective immediately, we will disable the upload of a number of Rekor entry types**, all of which
are unsupported by Sigstore client tooling and have single-digit uploads in a 90-day window.
...
Hardening the infrastructure and minimizing API surface reduces the risk of parsing vulnerabilities and
...
* **For Private Deployments:** These additional types continue to be supported in the Rekor codebase, so you may continue to use them, though we recommend migrating to use `hashedrekord`.
